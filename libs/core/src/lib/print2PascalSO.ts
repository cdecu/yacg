 ...
 */
export class Print2PascalSO<AMI> extends IntfModelPrintor implements intfModelPrintor<AMI> {
  /**
...
`;
  ...
  constructor(public readonly ami: intfModel<AMI>, public readonly config: any) {
    ...
    Handlebars.registerHelper("JDocDescr", (indent: number, val: string) => Print2PascalSO<AMI>.JDocDescr(indent, val));
  ...
  private assignTemplateScope(model: intfModel<AMI>, o: intfObjInfo<AMI>) {
    this.scope = {
      name: Print2PascalSO<AMI>.convertIntfName(o.name),
  ...
  private buildProperties(o: intfObjInfo<AMI>): unknown[] {
    return o.properties.map((property) => {
      return {
        ...property,
        name: Print2PascalSO<AMI>.convertPropertyName(property.name),
        decl: ":",
  ...
  private buildPropertyType(property: intfPropr<AMI>): string {
    ...
    return "variant" + " " + property.type + " " + property.subType;
  }
